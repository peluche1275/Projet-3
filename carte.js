...
run() {
  ...
  request.onreadystatechange = function () {
    ...
    if (this.readyState == XMLHttpRequest.DONE && this.status == 200) {  // Attente de la réponse positive du serveur.
      self.response = JSON.parse(this.responseText);
      console.log(self.response)
      for (let station in self.response) { // Créations des marqueurs.
